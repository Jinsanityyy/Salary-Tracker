...
function getRateColor(label) {
  ...
  return { t: "#94a3b8", bg: "rgba(56,189,248,0.08)", b: "rgba(56,189,248,0.18)" };
}
...
function StatusBadge({ isActual, isLocked }) {
  if (isLocked) return (
    <span style={{ ...BADGE_BASE, color: "#93c5fd", background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.35)" }}>
      📌 PAYSLIP
    </span>
  );
  if (isActual) return (
    <span style={{ ...BADGE_BASE, color: "#5eead4", background: "rgba(20,184,166,0.12)", border: "1px solid rgba(20,184,166,0.35)" }}>
      ✓ ACTUAL
    </span>
  );
  return (
    <span style={{ ...BADGE_BASE, color: "#4d7099", background: "rgba(12,24,52,0.9)", border: "1px solid rgba(255,255,255,0.1)" }}>
      · EST
    </span>
  );
}
...
function PBar({ value, max, color = "#14b8a6", animate = true, showPct = false }) {
  ...
  return (
    <div>
      <div style={{ background: "rgba(56,189,248,0.1)", borderRadius: 999, height: 7, overflow: "hidden", position: "relative" }}>
        <div style={{
...
function Chevron({ open }) {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4d7099" strokeWidth="2.5" strokeLinecap="round"
...
function CutoffCard({ title, income, items, carryOver, cardKey, onExtrasChange }) {
  ...
  return (
    <div style={{ background: "#070e20", border: "1px solid rgba(56,189,248,0.1)", borderRadius: 20, overflow: "hidden" }}>
      ...
      <div style={{ padding: "12px 18px", borderTop: "1px solid rgba(56,189,248,0.08)" }}>
        ...
        {showAdd ? (
          ...
          <button onClick={() => setShowAdd(true)} style={{ marginTop: 8, background: "none", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 8, padding: "7px 12px", color: "#3a5a80", fontSize: 11, cursor: "pointer", width: "100%", textAlign: "left" }}>
            + Add misc expense
          </button>
        )}

        <SubtotalRow label="Remaining after bills" value={afterBills} color={afterBills >= 0 ? "#cbd5e1" : "#fda4af"} />
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080d1c", color: "#e2e8f0", fontFamily: "'DM Sans', sans-serif", paddingBottom: "calc(76px + env(safe-area-inset-bottom, 0px))" }}>
          ...
          <div style={{ background: "#0f1623", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: 24, width: "100%", maxWidth: 400 }}
            ...
            {!supabase && (
              <div style={{ background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.25)", borderRadius: 12, padding: "12px 14px", marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "#fda4af", marginBottom: 6, fontWeight: 600 }}>Sync not configured</div>
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              {syncIdError && <div style={{ fontSize: 11, color: "#fda4af", marginTop: 6 }}>{syncIdError}</div>}
            </div>

            {supabase && (
              <button onClick={() => pushToCloud(syncId)}
                style={{ width: "100%", marginTop: 12, background: "rgba(20,184,166,0.1)", border: "1px solid rgba(20,184,166,0.3)", borderRadius: 10, padding: "11px", color: "#5eead4", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>
                ↑ Push to Cloud Now
              </button>
            )}

            <button onClick={() => setShowSyncModal(false)}
              style={{ width: "100%", marginTop: 8, background: "none", border: "1px solid rgba(56,189,248,0.12)", borderRadius: 10, padding: "10px", color: "#4d7099", fontSize: 12, cursor: "pointer" }}>
              Close
            </button>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
            ...
            <button className="btn" onClick={() => setUseCustomFx(p => !p)} style={{
              ...
              color: useCustomFx ? "#fcd34d" : "#4d7099" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "18px 18px 0" }}>
                  ...
                  <div key={group.mk}>
                    ...
                    <div onClick={() => toggleMonth(group.mk)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 6px 8px", cursor: "pointer", userSelect: "none" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 600, color: hasCurrent ? "#e2e8f0" : hasActual ? "#94a3b8" : "#4d7099" }}>
                          {group.label}
                        </span>
                        {hasCurrent && (
                          <span style={{ fontSize: 8, color: "#93c5fd", background: "rgba(59,130,246,.18)", border: "1px solid rgba(59,130,246,.35)", borderRadius: 99, padding: "1px 7px", animation: "pulse 2s infinite" }}>
                            CURRENT
                          </span>
                        )}
                        {actualCount > 0 && !hasCurrent && (
                          <span style={{ fontSize: 9, color: "#5eead4", opacity: 0.7 }}>{actualCount}/{group.entries.length} actual</span>
                        )}
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        {isCollapsed && (
                        ...
                        {group.entries.map(({ cycle, d, isNext }) => {
                          ...
                          return (
                            <div key={cycle.key}
                              ...
                              style={{
                                background: isNext ? "rgba(59,130,246,.08)" : "rgba(255,255,255,.02)",
                                border: `1px solid ${isNext ? "rgba(59,130,246,.38)" : d.isLocked ? "rgba(59,130,246,.2)" : d.isActual ? "rgba(20,184,166,.18)" : "rgba(255,255,255,.06)"}`,
                              ...
                              {!d.isLocked && (
                                <div style={{
                                  ...
                                  fontSize: 10, color: isEditing ? "#fda4af" : "#3b82f6",
                                  pointerEvents: "none",
                                }}>
                              ...
                              <div style={{ padding: "12px 42px 12px 14px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 6 }}>
                                  ...
                                  {isNext && (
                                    <span style={{ fontSize: 8, color: "#93c5fd", background: "rgba(59,130,246,.18)", border: "1px solid rgba(59,130,246,.38)", borderRadius: 99, padding: "1px 6px", animation: "pulse 2s infinite" }}>
                                      NEXT PAYOUT
                                    </span>
                                  )}
                                </div>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap", marginBottom: 8 }}>
                                  ...
                                  <span style={{ color: "#3a5a80", fontSize: 10 }}>·</span>
                                ...
                                return (
                                  <div onClick={e => e.stopPropagation()} style={{ background: "rgba(59,130,246,.06)", borderTop: "1px solid rgba(59,130,246,.18)", padding: "13px 14px" }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                                      ...
                                      <button className="btn" onClick={() => setEditing(null)} style={{ background: "none", border: "none", color: "#3a5a80", fontSize: 14, padding: "0 2px" }}>✕</button>
                                    </div>

                                    {/* Rate type */}
                                    <div style={{ marginBottom: 12 }}>
                                      <div style={{ fontSize: 9, color: "#4d7099", marginBottom: 6 }}>RATE TYPE</div>
                                      <div style={{ display: "flex", gap: 6 }}>
                                        {[
                                          { val: "mc",     label: "MC $3.75",    color: "#fcd34d", border: "rgba(251,191,36,.4)",  bg: "rgba(251,191,36,.12)" },
                                          { val: "client", label: "Client $5.50", color: "#5eead4", border: "rgba(20,184,166,.4)", bg: "rgba(20,184,166,.12)" },
                                          ...
                                          <button key={r.val} className="btn" onClick={() => handleEditChange("rateType", r.val)} style={{
                                            ...
                                            color: editVal.rateType === r.val ? r.color : "#4d7099",
                                          }}>
                                            {r.label}
                                          </button>
                                        ))}
                                    ...
                                    {isMixed && (
                                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 10, background: "rgba(167,139,250,.06)", border: "1px solid rgba(167,139,250,.18)", borderRadius: 9, padding: "10px 12px" }}>
                                        {[
                                          ...
                                          { label: "Client Hours", key: "clientHours", color: "#5eead4", placeholder: "e.g. 79.88" },
                                        ].map(f => (
                                          <div key={f.key}>
                                    ...
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                                      {[
                                        { label: "PHP Amount *", key: "php",    placeholder: "e.g. 24750",                  color: "#93c5fd" },
                                        { label: isMixed ? "USD (auto)" : "USD", key: "usd", placeholder: isMixed ? autoUSD || "auto" : "e.g. 400", color: "#5eead4" },
                                    ...
                                    <div style={{ display: "flex", gap: 8 }}>
                                      <button className="btn" onClick={() => saveActual(cycle.key, displayPhp)} style={{ background: "rgba(20,184,166,.15)", border: "1px solid rgba(20,184,166,.35)", borderRadius: 8, padding: "8px 18px", fontSize: 11, color: "#5eead4" }}>
                                        ✓ Save Payslip
                                      </button>
                                      {actuals[cycle.key] && (
                                        <button className="btn" onClick={() => removeActual(cycle.key)} style={{ background: "rgba(244,63,94,.1)", border: "1px solid rgba(244,63,94,.25)", borderRadius: 8, padding: "8px 16px", fontSize: 11, color: "#fda4af" }}>
                                          Remove
                                        </button>
                                      )}
                                    </div>
        ...
        {tab === "budget" && (
          <div className="fu" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: "rgba(20,184,166,0.06)", border: "1px solid rgba(20,184,166,0.2)", borderRadius: 16, padding: "18px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: 11, color: "#3a5a80" }}>
            ...
            />
            <div style={{ display: "flex", alignItems: "center", gap: 10, color: "#3a5a80", fontSize: 12 }}>
                ...
                return (
                  <div key={i} style={{ borderTop: "1px solid rgba(56,189,248,0.08)", paddingTop: 14, marginTop: i > 0 ? 14 : 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <div style={{ fontSize: 10, color: "#4d7099", whiteSpace: "nowrap" }}>{paidPct}% paid</div>
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              {[
                ...
                <div key={i} style={{ background: "rgba(10,20,45,0.8)", border: "1px solid rgba(56,189,248,0.1)", borderRadius: 14, padding: "16px 10px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 10, color: "#4d7099" }}>{r.desc}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          return (
            <div className="fu" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ background: "linear-gradient(145deg, rgba(20,184,166,0.1), rgba(59,130,246,0.08))", border: "1px solid rgba(20,184,166,0.2)", borderRadius: 20, padding: "22px 18px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#3a5a80" }}>
                  <span>₱0</span>
                  <span style={{ color: "#4d7099" }}>₱{monthlyRate.toLocaleString()}/mo × {monthsLeft} months</span>
                  <span>₱1M</span>
                </div>
              </div>

              {/* Milestones */}
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(56,189,248,0.1)", borderRadius: 16, padding: "16px 18px" }}>
                <div style={{ fontSize: 10, color: "#4d7099", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>Milestones</div>
                {MILESTONES.map((m, i) => {
              ...
              {/* Log entry form */}
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(56,189,248,0.1)", borderRadius: 16, padding: "16px 18px" }}>
                <div style={{ fontSize: 10, color: "#4d7099", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>Log a Savings Entry</div>

                {showAddSavings ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <input placeholder="Label (e.g. May C2 savings)"
                      value={newSavingsLabel} onChange={e => setNewSavingsLabel(e.target.value)}
                      style={{ background: "rgba(56,189,248,0.08)", border: "1px solid rgba(56,189,248,0.18)", borderRadius: 8, padding: "10px 12px", fontSize: 13, color: "#e2e8f0", outline: "none", width: "100%" }} />
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button onClick={() => { setShowAddSavings(false); setNewSavingsLabel(""); setNewSavingsAmt(""); }}
                        style={{ background: "none", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "10px 12px", color: "#4d7099", fontSize: 13, cursor: "pointer" }}>✕</button>
                    </div>
                  </div>
                ) : (
              ...
              {/* Savings log list */}
              {savingsLog.length > 0 && (
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(56,189,248,0.1)", borderRadius: 16, overflow: "hidden" }}>
                  <div style={{ padding: "14px 18px 10px", fontSize: 10, color: "#4d7099", letterSpacing: 2, textTransform: "uppercase" }}>History</div>
        ...
        {tab === "food" && (
          <div className="fu" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "rgba(10,20,45,0.8)", border: "1px solid rgba(56,189,248,0.1)", borderRadius: 16, padding: "20px 18px" }}>
              <div style={{ fontSize: 10, color: "#4d7099", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>Daily Food Limits</div>
            ...
            <div style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 16, padding: "20px 18px" }}>
              ...
              <div style={{ fontSize: 11, color: "#3a5a80", marginTop: 6 }}>vs ₱1,400/week danger zone</div>
            </div>

            <div style={{ background: "rgba(10,20,45,0.8)", border: "1px solid rgba(56,189,248,0.1)", borderRadius: 16, padding: "20px 18px" }}>
              <div style={{ fontSize: 10, color: "#4d7099", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>Budget Protein Swaps</div>
              {[
        ...
        {tab === "30-day plan" && (
          <div className="fu" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", gap: 8 }}>
              {[1, 2, 3, 4].map(w => (
                <button key={w} className="btn" onClick={() => setActiveWeek(w)} style={{
                  flex: 1,
                  background: activeWeek === w ? "rgba(59,130,246,0.2)" : "rgba(10,20,45,0.8)",
                  border: `1px solid ${activeWeek === w ? "#3b82f6" : "rgba(56,189,248,0.12)"}`,
                  borderRadius: 12, padding: "10px 0", fontSize: 12,
                  color: activeWeek === w ? "#93c5fd" : "#4d7099",
                }}>
                  Week {w}
                  <span style={{ display: "block", fontSize: 9, color: activeWeek === w ? "#3b82f6" : "#1e3a5f", marginTop: 2 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {weekTasks.map(task => (
                <div key={task.idx} className="task-row" onClick={() => toggleTask(task.idx)} style={{
                  ...
                  background: task.done ? "rgba(20,184,166,0.08)" : "rgba(10,20,45,0.8)",
                  border: `1px solid ${task.done ? "rgba(20,184,166,0.25)" : "rgba(56,189,248,0.1)"}`,
                  ...
                  <div style={{ fontSize: 13, color: task.done ? "#5eead4" : "#e2e8f0", lineHeight: 1.5, textDecoration: task.done ? "line-through" : "none", opacity: task.done ? 0.7 : 1 }}>
                    {task.label}
                  </div>
                </div>
              ))}
            ...
            <button className="btn" onClick={() => { if (confirm("Reset all tasks?")) { setBudgetTasks(BUDGET_DATA.tasks); showToast("Tasks reset"); } }} style={{ background: "rgba(10,20,45,0.8)", border: "1px solid rgba(56,189,248,0.12)", borderRadius: 10, padding: "9px 16px", fontSize: 11, color: "#3a5a80", alignSelf: "flex-start" }}>
              ↺ Reset all tasks
            </button>
          </div>
        )}
